...
setup_env() {
	...
	echo "CMD_K3S_EXEC:--> $CMD_K3S_EXEC"
...
#     fi
	HASH_URL=${INSTALL_PATH}/${VERSION_K3S}/${CHECK_SUM}
    info "Copying hash ${HASH_URL}"
    $SUDO cp -v ${HASH_URL} ${TMP_HASH} 
	info "Copying hash is completed"
	info "SUFFIX = ${SUFFIX}, ARCH = ${ARCH}"
...
#     fi
	info "Copying binary ${BIN_URL}"
	BIN_URL=${INSTALL_PATH}/${VERSION_K3S}/k3s
    
    $SUDO cp ${BIN_URL} ${TMP_BIN}
	info "Copying binary is completed"
}
